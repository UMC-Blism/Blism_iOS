...
class ReplyAPI {
    ...
    // 답장 디테일  조회
    func getDetailReply(request: ReadReplyDetailRequest, completion: @escaping (Result<ReadReplyDetailResponse, NetworkError>) -> Void){
    ...
    func replyLetter(image: UIImage, request: ReplyLetterRequest, completion: @escaping (Result<ReplyLetterResponse, NetworkError>) -> Void) {
        provider.request(.reply(image: image, request)) { response in
            switch response {
            case let .success(result):
                if 200..<400 ~= result.statusCode {
                    do {
                        let decodingResult = try JSONDecoder().decode(ReplyLetterResponse.self, from: result.data)
                        if decodingResult.isSuccess {
                            completion(.success(decodingResult))
                        } else {
                            print("kk")
